...
/**
 * time = minutesToTest / minutesToDie 每只🐷有time次喝水的机会，所以一只猪携带的信息熵是time+1
 * 两只猪携带的是 (time+1)^2,思考成二维的情况
 */
public class Solution {
    // 这题比较巧妙！！！
    public int poorPigs(int buckets, int minutesToDie, int minutesToTest) {
        int time = minutesToTest / minutesToDie;
        double temp = Math.log(buckets) / Math.log(time+1);
